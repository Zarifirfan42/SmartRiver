...
def predict_future_wqi(
    ...
    cfg = trained.get("config") or {}
    add_month = bool(cfg.get("add_month_cyclical", False))
    extra_params = tuple(cfg.get("extra_param_columns") or ())

    base = _normalize_columns(df)
    date_col = "date" if "date" in base.columns else "reading_date"
    if date_col not in base.columns:
        raise ValueError("DataFrame must have a date column (date or reading_date) for LSTM features")
    window = forecasting_service.build_prediction_window(
        base,
        station_code=station_code,
        seq_len=seq_len,
        date_col=date_col,
        add_month_cyclical=add_month,
        extra_param_columns=extra_params,
    )
    pred = forecasting_service.predict(model, scaler, window, horizon=horizon_days)
