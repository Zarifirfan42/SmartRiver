...
class PredictionAPI:
    ...
    def load_forecasting_model(self) -> bool:
        ...
        if not (path / "lstm_model.keras").exists() and not (path / "model.keras").exists():
            return False
    ...
    def predict_wqi_forecast(
        ...
        date_col = "date" if "date" in df.columns else ("reading_date" if "reading_date" in df.columns else None)
        if date_col is None:
            return None
        try:
            window = build_prediction_window(
                ...
                seq_len=seq_len,
                date_col=date_col,
                add_month_cyclical=bool(self._lstm_config.get("add_month_cyclical", False)),
                extra_param_columns=tuple(self._lstm_config.get("extra_param_columns") or ()),
            )
