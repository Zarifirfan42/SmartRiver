...
def user_row_email_verified(row: dict) -> bool:
    """False only for public users with email_verified=0. Admins are always treated as verified."""
    if _normalize_role(row) == "admin":
        return True
    v = row.get("email_verified")
    if v is None:
        return True
    try:
        return int(v) != 0
...
def public_user_from_row(row: dict) -> dict:
    ...
    return {
        ...
        "username": _text_or_none(row.get("username")),
        "full_name": _text_or_none(row.get("full_name")),
        "role": _normalize_role(row),
        "last_login_date": _text_or_none(row.get("last_login_date")),
        "email_verified": user_row_email_verified(row),
    }
