...
async def upload_dataset(
    ...
    from backend.db.repository import delete_dataset, save_dataset
    ...
    # Append semantics: register a new dataset row each time; readings merge by (station_code, date).
...
class ExportLogBody(BaseModel):
    """Audit entry when a user exports data (PDF/CSV)."""
    format: str
    data_range: str


@router.post("/export-log")
def log_data_export(body: ExportLogBody, user: dict = Depends(require_user)):
    from backend.db.repository import record_export_log

    return record_export_log(int(user["id"]), body.format, body.data_range)


@router.get("/export-logs")
def list_data_export_logs(
    limit: int = Query(200, ge=1, le=2000),
    _admin: dict = Depends(require_admin),
):
    from backend.db.repository import list_export_logs

    items = list_export_logs(limit=limit)
    return {"items": items, "total": len(items)}
