...
def dashboard_compare_series(
    station_codes: str = Query(
        ...,
        description="Comma-separated station codes, e.g. S01,S02,S03",
    ),
    year: int = Query(None, description="Optional filter year for historical series"),
    limit: int = Query(2000, ge=1, le=5000),
):
    """Multi-station WQI time series for one comparative chart."""
    parts = [p.strip() for p in (station_codes or "").split(",") if p.strip()]
    if len(parts) < 2:
        raise HTTPException(
            status_code=400,
            detail="Provide at least two station codes separated by commas (e.g. S01,S03).",
        )
    return get_compare_wqi_series(parts, year=year, limit=limit)


@router.get("/wqi-calendar")
def dashboard_wqi_calendar(
    station_code: str = Query(..., description="Station code"),
    year: int = Query(..., ge=1990, le=2035, description="Calendar year"),
):
    """Daily WQI cells for a GitHub-style heatmap (null when no reading)."""
    return get_wqi_calendar_heatmap(station_code, year)


@router.get("/parameter-contribution")
def dashboard_parameter_contribution(
    station_code: str = Query(...),
    date: str = Query(..., description="YYYY-MM-DD"),
):
    """
    Which parameters stressed WQI most on one day (SQLite water_quality_records with chemistry).
    """
    try:
        return get_parameter_contribution_breakdown(station_code, date)
    except ValueError as e:
        msg = str(e)
        if msg == "no_measured_row":
            raise HTTPException(
                status_code=404,
                detail="No measured row for this station and date. Pick a date present in uploaded chemistry data.",
            ) from e
        if msg == "no_parameters":
            raise HTTPException(
                status_code=422,
                detail="Row exists but DO/BOD/COD/AN/SS/pH are all empty.",
            ) from e
        raise HTTPException(status_code=400, detail="Invalid station or date (use YYYY-MM-DD).") from e
