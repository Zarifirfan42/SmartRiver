...
def historical_observation_cutoff_year() -> int:
    """
    Latest calendar year to load from CSV files as real observations.
    Uses the current year so e.g. 2026 WQI appears in time series; next year advances automatically.
    Rows with year > this are skipped (reserved for model forecast outputs, not raw CSV).
    """
