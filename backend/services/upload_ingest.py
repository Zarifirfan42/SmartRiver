...
def _numeric_series_fields(row, *col_names: str):
    """First present numeric column among col_names, or None."""
...
def _water_quality_sql_rows_from_df(df: pd.DataFrame, dataset_id: int, data_source: str) -> list[dict]:
    """Build rows for SQLite water_quality_records (historical uploads: is_predicted=False)."""
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            continue
        if y > MAX_DOE_FORMULA_CSV_YEAR:
            continue
        sc = str(row.get("station_code", "S01")).strip()
        sn = row.get("station_name")
        sn = str(sn).strip() if sn is not None and str(sn).strip() else None
        wqi_v = _numeric_series_fields(row, "WQI", "wqi")
        if wqi_v is None:
            wqi_v = 0.0
        pol = status_from_wqi(float(wqi_v))
        out.append(
            {
                "date": d,
                "station_code": sc,
                "station_name": sn,
                "DO": _numeric_series_fields(row, "DO"),
                "BOD": _numeric_series_fields(row, "BOD"),
                "COD": _numeric_series_fields(row, "COD"),
                "AN": _numeric_series_fields(row, "AN", "NH3-N"),
                "SS": _numeric_series_fields(row, "SS", "TSS"),
                "pH": _numeric_series_fields(row, "pH"),
                "WQI": float(wqi_v),
                "pollution_status": pol,
...
def ingest_upload_csv_path(
    ...
    *,
    ...
    from backend.db.repository import append_readings_with_dedup, upsert_water_quality_records
    ...
    try:
        ...
        df = add_wqi(df)
        # Drop forecast-window rows: 2026+ WQI must come from LSTM prediction_logs only.
        if "date" in df.columns:
            dt = pd.to_datetime(df["date"], errors="coerce")
            from data_preprocessing.utils.wqi_calculator import MAX_DOE_FORMULA_CSV_YEAR

            df = df.loc[dt.notna() & (dt.dt.year <= MAX_DOE_FORMULA_CSV_YEAR)].copy()
    ...
    try:
        sql_rows = _water_quality_sql_rows_from_df(
            df, int(dataset_id), data_source=f"csv_upload:{filename}"
        )
        upsert_water_quality_records(sql_rows)
    except Exception:
        logger.exception("persist water_quality_records skipped for %s", filename)
