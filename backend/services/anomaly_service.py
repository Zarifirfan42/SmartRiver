...
def run_anomaly_detection(
    ...
    import pandas as pd
    import joblib
    from backend.db.repository import save_prediction_log, save_alert, status_from_wqi
    ...
    if anomalies:
        ...
        for a in anomalies:
            sc = str(a.get("station_code") or "—").strip()
            sn = str(a.get("station_name") or sc).strip()
            wqi_v = a.get("wqi")
            try:
                wqf = float(wqi_v) if wqi_v is not None else None
            except (TypeError, ValueError):
                wqf = None
            rs = status_from_wqi(wqf) if wqf is not None else "slightly_polluted"
            reason = str(a.get("reason") or "Abnormal pattern").strip()
            d = str(a.get("date") or "")[:10]
            msg = f"Isolation Forest anomaly at {sn}: {reason}. Score {a.get('score', 0):.3f}."
            save_alert(
                station_code=sc,
                station_name=sn,
                message=msg,
                severity="critical",
                prediction_log_id=log.get("id"),
                wqi=wqf,
                date_str=d or None,
                alert_type="anomaly",
                river_status=rs,
