...
================================================================================
HISTORICAL WQI (DOE formula — manual computation from measurements)
================================================================================
For observations dated through calendar year 2025 (training/historical records),
WQI MUST be derived only from the official weighted sub-indices below — never from
an ad-hoc proxy or a single ML step output stored as ``readings``.

Forecast calendar year 2026 uses the trained LSTM elsewhere (``prediction_logs`` /
``backend.services.forecast_service``); those values MUST NOT be recomputed with this module.

================================================================================
ML-PREDICTED WQI (calendar year 2026 — DO NOT use this file)
================================================================================
Rows dated in 2026 are excluded from CSV ingestion into monitoring readings. Their WQI
comes exclusively from the LSTM forecast pipeline after training (see ``forecast_service``).

Classification thresholds (Malaysia river classes): Clean 81–100; Slightly polluted 60–80;
Polluted 0–59 (implemented in ``wqi_to_status`` / ``repository.status_from_wqi``).
"""
import numpy as np
import pandas as pd


# Last calendar year for which CSV uploads / readings may carry formula-derived WQI.
# Dates in the ML forecast window year must use LSTM outputs only (see project constants).
MAX_DOE_FORMULA_CSV_YEAR = 2025
...
WQI_WEIGHTS = {
    ...
    "TSS": 0.16,  # Total suspended solids (maps from SS / SI_SS in field sheets)
    "pH": 0.12,
}
...
def wqi_to_status(wqi: float) -> str:
    """Map WQI to river class: Clean 81–100; Slightly polluted 60–80; Polluted 0–59 (DOE-style bands)."""
    if pd.isna(wqi):
        return "unknown"
    w = float(wqi)
    if w >= 81:
...
def add_wqi_and_status(df: pd.DataFrame, wqi_column: str = "WQI") -> pd.DataFrame:
    ...
    out = df.copy()
    computed = compute_wqi(out).clip(0.0, 100.0)
